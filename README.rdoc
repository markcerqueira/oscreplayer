...
Playing back OSC messages with *oscplayer* requires specifying the address (--address, -a) and port (--port, -p) to send OSC messages to and optionally, a flag (--wait, -w) to specify whether the padding for the initial message - the time from when recording began to when the first message was received - should be respected, and a filename (--filename, -f) to read data from. If no boolean is specified, the initial time padding is ignored. If no filename is specified, oscrecorder will attempt to load data from stdin.
...
2. GUI a la {Charles Proxy}[http://www.charlesproxy.com/] that allows for real-time recording and playing back with data modifications.
